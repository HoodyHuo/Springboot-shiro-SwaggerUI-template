...
import tech.hoody.platform.shiro.SignOutException
...
//        model.addAttribute("error", "用户帐号/密码不正确")
    ...
    @ExceptionHandler(value = SignOutException.class)
    ResponseData signOutException(SignOutException e, Model model) {
        model.addAttribute("error", "当前会话超时，或账号异地登录")
        return new ResponseData(code: 40305, msg: "当前会话超时，或账号异地登录")
    }
